...
/** Read the child's live event log. A continuable child's session is often
 *  not in the ROOT SessionStore after creation (the continuation manager
 *  holds it), so fall back to the child agent's own scoped ctx. */
function readChildEvents(hostCtx, childId) {
...
function waitForTurn(hostCtx, childId, lastSeq) {
  return new Promise((resolve) => {
    ...
    const timer = setInterval(() => {
      try {
        const read = readChildEvents(hostCtx, childId);
      ...
      if (Date.now() - started > TIMEOUT_MS) {
        clearInterval(timer);
        const diag = readChildEvents(hostCtx, childId);
          ...
          handler: async (request, response) => {
            ...
            try {
              ...
              if (!entry) {
                ...
                const fresh = readChildEvents(hostCtx, entry.childId);
              ...
              const sessRead = readChildEvents(hostCtx, entry.childId);
