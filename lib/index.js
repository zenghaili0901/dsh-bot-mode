        ...
        hostCtx.webServer.register({
          ...
          handler: async (request, response) => {
            ...
            try {
              ...
              if (!outcome.timedOut && outcome.blocks.length === 0) {
                console.warn('[bot-mode] 回合完成但无输出块', { childId: entry.childId, ...outcome });
              }
              sendJson(response, 200, { ok: true, ...parseOutput(outcome.blocks) });
              });
            } catch (err) {
