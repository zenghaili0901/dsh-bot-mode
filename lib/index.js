...
function parseOutput(blocks) {
  ...
  reply = reply.replace(UI_FENCE, (_, json) => {
    try {
      ...
      /* unparseable fence: keep the original text so nothing is lost */
      return _.replace(/^```dsh-ui\s*/, '').replace(/```$/, '');
    }
  });
...
/** Read the child's event log through three paths, most reliable first for a
 *  continuable child: (1) root SessionStore, (2) the child agent's scoped ctx,
 *  (3) sessionPersistence.inspect — the DURABLE log, which always has the
 *  events even when the live store no longer serves the child. */
async function readChildEvents(hostCtx, childId) {
  const s1 = hostCtx.sessions.get(childId);
  if (s1 && Array.isArray(s1.events)) return { events: s1.events, sess: s1 };
  try {
    const agent = hostCtx.agents.get(childId);
    if (agent && agent.ctx && agent.ctx.sessions) {
      const s2 = agent.ctx.sessions.get(childId);
      if (s2 && Array.isArray(s2.events)) return { events: s2.events, sess: s2 };
    }
  } catch {}
  try {
    const insp = await hostCtx.sessionPersistence.inspect(childId, AbortSignal.timeout(3000));
    if (insp && Array.isArray(insp.events)) return { events: insp.events, sess: null };
  } catch {}
  return null;
}

/** Poll until this round's turn/end appears (async recursion so we can await
 *  the durable inspect path). Returns { blocks, timedOut, turnEndSeq, lastSeq,
 *  seen, maxSeq } for diagnosis. */
function waitForTurn(hostCtx, childId, lastSeq) {
  return new Promise((resolve) => {
    const started = Date.now();
    const check = async () => {
      try {
        const read = await readChildEvents(hostCtx, childId);
        ...
        if (turnEndSeq !== -1) {
          ...
          resolve({ blocks, timedOut: false, turnEndSeq, lastSeq, seen: events.length, maxSeq: maxEventSeq(read ? read.sess : null) });
          return;
        }
      } catch {}
      if (Date.now() - started > TIMEOUT_MS) {
        const diag = await readChildEvents(hostCtx, childId).catch(() => null);
        console.warn('[bot-mode] waitForTurn 超时', {
          childId, lastSeq,
          seen: diag ? diag.events.length : -1,
          maxSeq: diag ? maxEventSeq(diag.sess) : 0,
...
export function apply(ctx, config) {
  ctx.inject(['webServer', 'subagents', 'agents', 'sessions', 'sessionPersistence'], (hostCtx) => {
    hostCtx.effect(() => {
          ...
          handler: async (request, response) => {
            ...
            try {
              ...
              if (!entry) {
                ...
                const fresh = await readChildEvents(hostCtx, entry.childId);
                entry.lastSeq = maxEventSeq(fresh ? fresh.sess : null);
                continuable.set(bot.id, entry);
              } else {
                /* followup options carry only source + signal — the model is
                   fixed at startContinuable (agentOptions), not changeable per
                   followup; a model change requires a child rebuild */
              ...
              const sessRead = await readChildEvents(hostCtx, entry.childId);
              const sess = sessRead ? sessRead.sess : null;
