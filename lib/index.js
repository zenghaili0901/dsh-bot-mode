        ...
        hostCtx.webServer.register({
          ...
          handler: async (request, response) => {
            ...
            try {
              ...
              if (!bot || typeof bot.id !== 'string' || !bot.id || !bot.name
                || typeof bot.name !== 'string' || bot.name.length > 60
          ...
          handler: async (request, response) => {
            ...
            try {
              ...
              if (typeof botId === 'string') {
                /* release the in-memory continuable entry + lock when a bot
                   is deleted (the child session itself stays — no public
                   session-delete API — but stops growing) */
