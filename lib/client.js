...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      ".bm-capsule{display:flex;align-items:center;gap:6px;border:1px solid var(--dsw-alias-border-l2);border-radius:10px;background:color-mix(in srgb,var(--dsw-alias-bg-layer-2) 95%,transparent);backdrop-filter:blur(14px);box-shadow:0 6px 18px rgba(2,6,20,.25);padding:4px 10px 4px 4px;cursor:pointer;font-size:10.5px;color:var(--dsw-alias-label-primary);transition:transform .15s,border-color .15s,box-shadow .15s}",
    ...
    function BotAvatar(props) {
      ...
      return h("span", { className: className || "bm-chip-avatar", style: { width: sz, height: sz, flex: "none", color: color || "var(--dsw-alias-label-secondary)" } },
        renderIcon("bot", Math.round(sz * 0.55)));
    }
