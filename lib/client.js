...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      ".bm-msg-img{max-width:100%;max-height:220px;border-radius:8px;margin:4px 0;cursor:zoom-in;display:block}",
    ...
    /* ---------------- role icon pool (official primitives) ----------------
       ...
       NOTE: ids are stable for backwards compatibility — bots persist the role
       id in localStorage, so renaming an id orphans existing bot avatars
       (they fall back to ROLE_POOL[0]). Keep legacy ids (spark, persona). */
    const ROLE_POOL = [
      ...
      { id: "dark", Icon: primitives.IconDarkOutline16, color: "#6366f1" },
      { id: "follow", Icon: primitives.IconFollowsystemOutline16, color: "#0ea5e9" },
      { id: "like", Icon: primitives.IconLikeOutline16, color: "#f43f5e" },
      { id: "edit", Icon: primitives.IconEditOutline16, color: "#7b5cff" },
      { id: "light", Icon: primitives.IconLightOutline16, color: "#eab308" },
      { id: "api", Icon: primitives.IconApiOutline14, color: "#22c55e" },
    ];
    function roleOf(id) {
    ...
    function IconTrash() { return h(primitives.IconTrashOutline16, { size: 18 }); }

    /* ---------------- persistence ---------------- */
    const BOTS_KEY = "dsh-bot-mode.bots";
    const CHATS_KEY = "dsh-bot-mode.chats";      /* legacy single-key store (compat) */
    const CHATS_PREFIX = "dsh-bot-mode.chats.";  /* per-bot store — incremental writes */
    ...
    function loadBots() {
      ...
      /* an empty array is a valid state (user deleted every bot) — do NOT
         resurrect the defaults on refresh; only fall back when unset */
      return Array.isArray(arr) ? arr : DEFAULT_BOTS;
    }
    function loadChats() {
      const chats = {};
      const legacy = loadJSON(CHATS_KEY, null);
      if (legacy && typeof legacy === "object") Object.assign(chats, legacy);
      try {
        for (let i = 0; i < localStorage.length; i++) {
          const k = localStorage.key(i);
          if (k && k.startsWith(CHATS_PREFIX)) {
            const botId = k.slice(CHATS_PREFIX.length);
            try {
              const arr = JSON.parse(localStorage.getItem(k));
              if (Array.isArray(arr)) chats[botId] = arr;
            } catch {}
          }
        }
      } catch {}
    ...
    const uiStore = defineStore({
      ...
      actions: {
        ...
        removeBot: (d, id) => {
          ...
          delete chats[id];
          d.chats = chats;
          try { localStorage.removeItem(CHATS_PREFIX + id); } catch {}
          const wins = Object.assign({}, d.chatWins);
          delete wins[id];
          d.chatWins = wins;
          const busy = Object.assign({}, d.busyIds);
          delete busy[id];
          d.busyIds = busy;
        ...
        appendMessage: (d, botId, msg) => {
          /* ignore late replies for deleted bots (in-flight request arriving
             after removeBot would otherwise resurrect its chat) */
          if (!d.bots.some((b) => b.id === botId)) return;
          ...
          /* reasoning is a visual hint only — keep it in memory for the live
             window, drop it from persistence (refresh = gone); persist only
             this bot's history (incremental, no full-rewrite) */
          const persist = list.map((m) => {
            const copy = Object.assign({}, m);
            delete copy.reasoning;
            return copy;
          });
          saveJSON(CHATS_PREFIX + botId, persist);
        },
        ...
        /* reserved: cross-tab state sync surface (not wired yet — a later
           milestone can broadcast store snapshots through this action) */
    ...
    /* ---------------- helpers ---------------- */
    /* inline markdown images (![alt](src)) — render as <img>, keep the rest
       as plain text (lightweight; full markdown stays out of scope) */
    function renderInline(text) {
      if (!text || text.indexOf("![") === -1) return text;
      /* only safe schemes render as <img>; anything else stays plain text
         (blocks javascript: URLs from becoming a self-XSS open window) */
      const SAFE_SRC = /^(data:image\/|https?:|blob:|about:)/i;
      const parts = [];
      const re = /!\[([^\]]*)\]\(([^)]+)\)/g;
      let last = 0;
      let m;
      ...
      while ((m = re.exec(text)) !== null) {
        if (m.index > last) parts.push({ k: "t" + idx++, text: text.slice(last, m.index) });
        if (SAFE_SRC.test(m[2])) parts.push({ k: "i" + idx++, alt: m[1] || "", src: m[2] });
        else parts.push({ k: "t" + idx++, text: m[0] });
        last = m.index + m[0].length;
      }
      if (last < text.length) parts.push({ k: "t" + idx++, text: text.slice(last) });
      return parts.map((p) =>
        p.k[0] === "i"
          ? h("img", { key: p.k, src: p.src, alt: p.alt, className: "bm-msg-img", onClick: () => {
              try { const w = window.open(p.src, "_blank", "noopener"); if (w) w.opener = null; } catch {}
            } })
          : h("span", { key: p.k }, p.text)
      );
    }
    ...
    function BotAvatar(props) {
      ...
      const sz = size || 36;
      /* legacy-data compatibility: bots created before the icon system carry
         an emoji avatar — render it until the data is re-created */
      if (emoji && !icon) {
        ...
        searching
          ...
          : (bots.length === 0
              ? h("div", { style: { display: "flex", flexDirection: "column", gap: 8, flex: 1 } },
                  h("div", { className: "bm-empty" }, "还没有 Bot ✨\n点下方 + 新建你的第一个团队成员。"),
                  /* empty state still needs the create entry point */
              ...
              : h("div", { className: "bm-grid" },
                  ...
                  h("button", { key: "__del", className: "bm-chip bm-chip-new" + (deleteMode ? " on" : ""), onClick: () => { setDeleteMode(!deleteMode); setConfirmBot(null); }, title: deleteMode ? "退出删除模式" : "删除 Bot" },
                    h("span", { className: "bm-chip-avatar" }, h(IconTrash)),
    ...
    function UIRender(props) {
      ...
      return h("div", { className: "bm-ui" },
        ...
        spec.items.map((item, i) => renderUiItem(item, i, 0))
      );
    }
    function renderUiItem(item, key, depth) {
      /* recursion guard: hostile / malformed specs cannot overflow the stack */
      ...
      switch (item.type) {
        ...
        case "keyvalue": {
          ...
          const itemList = Array.isArray(item.pairs)
            ...
            : Array.isArray(item.rows)
              ...
              : Array.isArray(item.items) && Array.isArray(item.items[0])
                ? item.items.map((r) => ({ key: r && r[0], value: r && r[1] }))   /* items: [[k,v]] */
                : Array.isArray(item.items) && item.items[0] && ("key" in item.items[0] || "value" in item.items[0])
                  ? item.items                                                       /* items: [{key,value}] */
        ...
        case "mermaid":
          /* lightweight: show the diagram source (no mermaid.js dependency) */
          return h("div", { key, className: "bm-ui-mermaid" },
            item.chart ? h("div", { className: "bm-ui-mermaid-tag" }, "mermaid · " + item.chart) : null,
            h("pre", { className: "bm-ui-mermaid-code" }, item.code || "")
          );
        case "steps":
        ...
        case "grid":
          return h("div", { key, style: { display: "flex", flexWrap: "wrap", gap: 6 } },
            (item.items || []).slice(0, 12).map((g, j) => h("div", { key: j, style: { flex: "1 1 45%", minWidth: 90 } }, renderUiItem(g, j, (depth || 0) + 1)))
          );
        default:
          return null;
      }
    ...
    function BotChatWindow(props) {
      ...
      const clamp = (r) => {
        ...
        /* also clamp size so the window can never exceed the viewport
           (composer stays on-screen after shrinking the browser) */
        const w = Math.min(Math.max(r.w, 260), Math.max(260, vw - 16));
        const h = Math.min(Math.max(r.h, 200), Math.max(200, vh - 16));
        if (x === r.x && y === r.y && w === r.w && h === r.h) return r;
        const next = Object.assign({}, r, { x, y, w, h });
      ...
      /* throttle localStorage persistence while dragging/resizing (120ms
         coalescing) — the rect state updates live, the write does not */
      ...
      /* unmount-safe drag/resize: a cleanup registered with the component so
         in-flight window listeners cannot leak if the window is disposed
         mid-drag */
      ...
      const onTitleDown = (e) => {
        ...
        const up = () => { window.removeEventListener("mousemove", move); window.removeEventListener("mouseup", up); dragCleanupRef.current = null; };
      ...
      return h("div", { className: "bm-chat-win", style: { left: safeRect.x, top: safeRect.y, width: rect.w, height: rect.h, zIndex: active ? 10001 : 10000 }, onMouseDown: onWindowMouseDown },
                      ...
                      h("div", { style: { flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 4 } },
                        h("div", { className: "bm-msg bm-msg-bot" + (m.error ? " bm-msg-error" : "") },
                          ...
                          m.tools && m.tools.length
                            ...
                            : null,
                          m.text ? renderInline(m.text) : null
                        ),
                        m.ui && m.ui.length ? m.ui.map((s, j) => h(UIRender, { key: "ui" + j, spec: s })) : null
                      )
                    )
                  : h("div", { key: i, className: "bm-msg bm-msg-user" }, renderInline(m.text))
              ),
        ...
        h("div", { className: "bm-composer" },
          h("input", {
            ...
            onKeyDown: (e) => { if (e.key === "Enter" && !e.shiftKey && !(e.nativeEvent && (e.nativeEvent.isComposing || e.nativeEvent.keyCode === 229))) { e.preventDefault(); send(); } },
          }),
    ...
    function NewBotModal(props) {
      ...
      const submit = () => {
        const n = name.trim();
        if (!n) return;
        props.onCreate({ id: "bot-" + Date.now() + "-" + Math.random().toString(36).slice(2, 7), name: n, icon, color: roleOf(icon).color, persona: persona.trim() || "你是一位乐于助人的助手。", createdAt: Date.now() });
      };
    ...
    let applied = false;
    const NS = "settings.botmode";
    function apply(ctx) {
      /* idempotent under HMR / re-apply: never double-register slots */
      if (applied) return;
      applied = true;
      /* serve a settings namespace so the Plugins-tab card (keyed by it) is
         dispatched — the tab renders the intersection of served namespaces
         and registered card keys */
      try {
        ctx.locale.register(NS, {
          zh: { "botmode.title": "Bot Mode", "botmode.description": "把子 Agent 变成一群有名字、有性格的 Bot" },
          en: { "botmode.title": "Bot Mode", "botmode.description": "Turn subagents into a roster of named bots" },
        });
      } catch (e) { console.warn("[bot-mode] locale 注册失败:", e && e.message); }
