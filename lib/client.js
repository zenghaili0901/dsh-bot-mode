...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      ".bm-overlay{position:fixed;bottom:80px;width:246px;max-height:56vh;display:flex;flex-direction:column;border:1px solid var(--dsw-alias-border-l2);border-radius:12px;background:color-mix(in srgb,var(--dsw-alias-bg-layer-2) 96%,transparent);backdrop-filter:blur(20px);box-shadow:0 12px 40px rgba(2,6,20,.3);z-index:10003;overflow:hidden;pointer-events:auto;font-size:13px;color:var(--dsw-alias-label-primary)}",
      ...
      /* confirm dialog */
      ".bm-confirm{position:fixed;inset:0;background:rgba(2,6,20,.4);backdrop-filter:blur(2px);z-index:10004;display:flex;align-items:center;justify-content:center;pointer-events:auto}",
              ...
              : h("div", { className: "bm-grid" },
                  bots.map((bot) =>
                    h("button", {
                      key: bot.id,
                      className: "bm-chip"
                        + (deleteMode && confirmBot && confirmBot.id === bot.id ? " bm-chip-del" : ""),
