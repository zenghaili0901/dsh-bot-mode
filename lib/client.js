...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      ".bm-overlay{position:fixed;bottom:64px;width:246px;max-height:56vh;display:flex;flex-direction:column;border:1px solid var(--dsw-alias-border-l2);border-radius:12px;background:color-mix(in srgb,var(--dsw-alias-bg-layer-2) 96%,transparent);backdrop-filter:blur(20px);box-shadow:0 12px 40px rgba(2,6,20,.3);z-index:10003;overflow:hidden;pointer-events:auto;font-size:13px;color:var(--dsw-alias-label-primary)}",
      ...
      /* delete mode: shake + red minus badge, iphone-style */
      ".bm-chip-del{animation:bm-shake .45s ease-in-out infinite}",
      "@keyframes bm-shake{0%,100%{transform:translateX(0)}25%{transform:translateX(-1.5px)}75%{transform:translateX(1.5px)}}",
      ...
      ".bm-modal-card{width:400px;max-width:92vw;max-height:84vh;overflow-y:auto;display:flex;flex-direction:column;gap:10px;border:1px solid var(--dsw-alias-border-l2);border-radius:14px;background:var(--dsw-alias-bg-layer-2);box-shadow:0 24px 60px rgba(2,6,20,.5);padding:16px;font-size:13px;color:var(--dsw-alias-label-primary)}",
      ...
      ".bm-btn{border:1px solid transparent;background:linear-gradient(135deg,#4d6bfe,#7b5cff);color:#fff;border-radius:8px;padding:7px 13px;font-size:12px;cursor:pointer;font-weight:600}",
    ...
    function IconMinus() { return h("svg", { width: 13, height: 13, viewBox: "0 0 12 12", fill: "none", stroke: "currentColor", strokeWidth: 1.3, strokeLinecap: "round" }, h("path", { d: "M2 6h8" })); }
    ...
    /* avatar: line icon, no disc background, fixed size (not squashed by flex) */
    function BotAvatar(props) {
      ...
      return h("span", { className: className || "bm-chip-avatar", style: { width: sz, height: sz, flex: "none", color: color || "var(--dsw-alias-label-secondary)" } },
        renderIcon("bot", Math.round(sz * 0.62)));
    }
    ...
    function BotModeOverlay(props) {
      ...
      const panelStyle = wide
        ? { right: (typeof window !== "undefined" ? window.innerWidth : 0) - sbW + 10, width: panelW }
        : { left: sbW + 8, width: panelW };
      /* capsule row is LEFT-aligned inside the sidebar (dock-like), roster stays right-aligned */
      const capStyle = wide ? { left: 10 } : { left: sbW + 8 };
      ...
      return h("div", null,
        ...
        minBots.length > 0 ? h("div", { className: "bm-caprow", style: capStyle },
          minBots.map((botId) => {
            const bot = state.bots.find((b) => b.id === botId);
            if (!bot) return null;
    ...
    function BotRoster(props) {
      ...
      /* iphone-style delete mode: chip click confirms after shake+red minus */
      const handleChipClick = (bot) => {
        if (!deleteMode) { onPick(bot); return; }
        if (confirmId === bot.id) { onRemove(bot.id); setConfirmId(null); setDeleteMode(false); }
              ...
              : h("div", { className: "bm-grid" },
                  bots.map((bot) =>
                    h("button", {
                      key: bot.id,
                      className: "bm-chip"
                        + (deleteMode ? " bm-chip-del" : "")
                        + (confirmId === bot.id ? " bm-chip-confirm" : ""),
                      onClick: () => handleChipClick(bot),
                      title: deleteMode ? (confirmId === bot.id ? "再点一次确认删除" : "点击删除此 Bot") : bot.name,
                    },
                      ...
                      h("span", { className: "bm-chip-name" }, confirmId === bot.id ? "确认删除？" : bot.name)
                    )
                  ),
                  /* trailing chips: separator + new + delete */
                  ...
                  h("button", { key: "__del", className: "bm-chip bm-chip-new" + (deleteMode ? " on" : ""), onClick: () => { setDeleteMode(!deleteMode); setConfirmId(null); }, title: deleteMode ? "退出删除模式" : "删除 Bot" },
        ...
        h("div", { className: "bm-modal-card" },
          ...
          h("div", { className: "bm-field" },
            ...
            h("div", { className: "bm-avatar-pick" },
              BOT_ICON_IDS.map((id) =>
                h("span", { key: id, className: "bm-chip-avatar" + (id === icon ? " sel" : ""), style: { width: 34, height: 34, color: id === icon ? "var(--dsw-alias-brand-primary)" : "var(--dsw-alias-label-secondary)" }, onClick: () => setIcon(id) },
                  renderIcon(id, 18))
              )
            )
          ),
