...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      ".bm-overlay{position:fixed;bottom:52px;width:246px;max-height:56vh;display:flex;flex-direction:column;border:1px solid var(--dsw-alias-border-l2);border-radius:12px;background:color-mix(in srgb,var(--dsw-alias-bg-layer-2) 96%,transparent);backdrop-filter:blur(20px);box-shadow:0 12px 40px rgba(2,6,20,.3);z-index:10003;overflow:hidden;pointer-events:auto;font-size:13px;color:var(--dsw-alias-label-primary)}",
          ...
          : (bots.length === 0
              ...
              : h("div", { className: "bm-grid" },
                  ...
                  h("button", { key: "__del", className: "bm-chip bm-chip-new" + (deleteMode ? " on" : ""), onClick: () => { setDeleteMode(!deleteMode); setConfirmId(null); }, title: deleteMode ? "退出删除模式" : "删除 Bot" },
                    ...
                    h("span", { className: "bm-chip-name" }, "删除")
                  )
                ))
      );
    }
