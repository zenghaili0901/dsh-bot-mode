  ...
  factory: (require) => {
                  ...
                  bots.map((bot) =>
                    h("button", {
                      ...
                      h("span", { className: "bm-chip-avatar", style: { position: "relative" } },
                        ...
                        h("span", { className: "bm-status-dot " + statusClass(!!busyIds[bot.id] ? "running" : "idle") })
                      ),
          ...
          return h("div", { key, className: "bm-ui-kv" },
            (item.pairs || []).map((p, j) =>
              h("div", { key: j, className: "bm-ui-kv-row" },
                h("span", { className: "bm-ui-kv-k" }, p.key || ""),
        ...
        case "table": {
          ...
          return h("table", { key, className: "bm-ui-table" },
            ...
            h("tbody", null, rows.map((r, j) =>
              h("tr", { key: j }, cols.map((c, k) => h("td", { key: k }, r && r[k] != null ? r[k] : "")))
            ))
          );
        }
        case "callout":
          return h("div", { key, className: "bm-ui-callout" },
            ...
            item.content ? h("div", { className: "bm-ui-callout-c" }, item.content) : null
          );
        case "stat":
    ...
    function BotChatWindow(props) {
      ...
      const send = async () => {
        ...
        setBusy(bot.id, true);
        try {
          /* send recent history so the bot keeps conversation context
             (fixes amnesia where it re-introduces itself on follow-ups) */
          const history = messages.slice(-10).map((m) => ({ role: m.role, text: m.text }));
          const res = await fetch("/bot-mode/chat", {
            method: "POST",
            headers: { "content-type": "application/json" },
            body: JSON.stringify({ sessionId, bot, message: text, history }),
          });
          const data = await res.json();
          const ok = data && data.ok && typeof data.reply === "string" && data.reply.trim().length > 0;
          const reply = data && data.ok
            ? (data.reply && data.reply.trim() ? data.reply : "(Bot 没有产生文本回复，请重试或换一种问法)")
          ...
          onAppend({ role: "bot", text: "请求失败: " + (err.message || err), ts: Date.now(), done: false });
        }
        setSending(false);
      ...
      return h("div", { className: "bm-chat-win", style: { left: rect.x, top: rect.y, width: rect.w, height: rect.h, zIndex: active ? 10001 : 10000 }, onMouseDown: onWindowMouseDown },
