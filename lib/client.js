...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      /* confirm dialog */
      ".bm-confirm{position:fixed;inset:0;background:rgba(2,6,20,.3);backdrop-filter:blur(1.5px);z-index:10004;display:flex;align-items:center;justify-content:center;pointer-events:auto}",
          ...
          : (bots.length === 0
              ...
              : h("div", { className: "bm-grid" },
                  bots.map((bot, i) =>
                    h("button", {
                      key: bot.id,
                      className: "bm-chip" + (deleteMode ? " bm-chip-del" : ""),
                      style: deleteMode ? { animationDelay: (i * 0.07) + "s" } : undefined,
