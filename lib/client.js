...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function BotModeOverlay(props) {
      ...
      return h("div", null,
        /* roster panel — anchored above the entry, right edge at sidebar edge, top layer */
        state.overlayOpen ? h("div", { className: "bm-overlay", style: { left } },
        ...
        /* standby capsule row — minimized chats, one line above the entry */
        minBots.length > 0 ? h("div", { className: "bm-caprow", style: { left } },
          minBots.map((botId) => {
            const bot = state.bots.find((b) => b.id === botId);
            if (!bot) return null;
